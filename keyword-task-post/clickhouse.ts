...
export const updateByKeywordIds = async (client: ClickHouseClient, keyword_ids: string[], dfs_task_id: string | undefined): Promise<any> => {
    await client.query({
        query: `ALTER TABLE keywords UPDATE dfs_task_id = '${dfs_task_id}' WHERE id IN (${keyword_ids.join(',')})`,
    });
}