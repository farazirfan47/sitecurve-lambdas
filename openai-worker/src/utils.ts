...
export const generateSerpPrompt = (pageMeta: any) => {
    let websiteMeta = "Title: " + pageMeta.title + "\n" + "Description: " + pageMeta.description;
    let prompt  = "Analyse the following wesbite meta and determine website type and business model from the predefined list of types and business models";
    prompt += "\n\nWebsite Meta: \n" + websiteMeta;
    ...
    prompt += "\n\nOutput the results in the following JSON format: {websiteTypes: 'website types array', businessModels: 'business models array'}";
    return prompt;
}

export const generateKeywordTagPrompt = (keyword: any) => {
    let prompt = "Analyse the following keyword and determine category and niche from the predefined list of categories and niches";
...
export const tagSerpDomains = async (mongoClient: any, domainResults: any) => {
    ...
    let bulkOps: any = [];
    domainResults.forEach((domainResult: any) => {
        bulkOps.push({
            updateOne: {
                filter: { "serps._id": domainResult._id },
                update: { $set: { "serps.$.website_types": domainResult.res.website_types, "serps.$.business_models": domainResult.res.business_models } }
            }
        });
    });
...
export const saveTaggedKeywords = async (mongoClient: any, keywordResults: any) => {
    const db = mongoClient.db("sitecurve");
    const keywords = db.collection("keywords");
    let bulkOps: any = [];
    keywordResults.forEach((keywordResult: any) => {
        bulkOps.push({
            updateOne: {
                filter: { _id: keywordResult._id },
                update: { $set: { category: keywordResult.res.category, niche: keywordResult.res.niche } }
            }
        });
    });
